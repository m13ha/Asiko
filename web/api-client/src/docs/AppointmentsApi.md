...
> EntitiesAppointment getAppointmentByAppCode(appCode)
...
import type { GetAppointmentByAppCodeRequest } from '';
...
async function example() {
  ...
  } satisfies GetAppointmentByAppCodeRequest;

  try {
    const data = await api.getAppointmentByAppCode(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
|-------------|-------------|------------------|
| **200** | OK |  -  |
| **400** | Missing app_code parameter |  -  |
