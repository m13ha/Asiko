...
func (r *gormAnalyticsRepository) GetAppointmentsByTypeCounts(userID uuid.UUID, startDate, endDate time.Time) (map[string]int, error) {
    type row struct{ Type string; Cnt int }
    var rows []row
    err := r.db.Table("appointments").
    ...
    out := map[string]int{}
    for _, v := range rows { out[v.Type] = v.Cnt }
    return out, nil
}
...
func (r *gormAnalyticsRepository) GetBookingsByStatusCounts(userID uuid.UUID, startDate, endDate time.Time) (map[string]int, error) {
    type row struct{ Status string; Cnt int }
    var rows []row
    err := r.db.Table("bookings").
        Select("bookings.status as status, COUNT(*) as cnt").
        Joins("JOIN appointments ON bookings.appointment_id = appointments.id").
        Where("appointments.owner_id = ? AND bookings.created_at BETWEEN ? AND ?", userID, startDate, endDate).
        Group("bookings.status").
        Find(&rows).Error
    if err != nil { return nil, err }
    out := map[string]int{}
    for _, v := range rows { out[v.Status] = v.Cnt }
    return out, nil
}
...
func (r *gormAnalyticsRepository) GetGuestVsRegisteredCounts(userID uuid.UUID, startDate, endDate time.Time) (map[string]int, error) {
    type row struct{ Kind string; Cnt int }
    var rows []row
    err := r.db.Table("bookings").
    ...
    if err != nil { return nil, err }
    out := map[string]int{}
    for _, v := range rows { out[v.Kind] = v.Cnt }
    return out, nil
}
...
func (r *gormAnalyticsRepository) GetDistinctAndRepeatCustomers(userID uuid.UUID, startDate, endDate time.Time) (int, int, error) {
    ...
    type rec struct{ Key string; Cnt int }
    var rows []rec
    err := r.db.Raw(`
        ...
        WHERE appointments.owner_id = ? AND bookings.created_at BETWEEN ? AND ?
        GROUP BY key
    `, userID, startDate, endDate).Scan(&rows).Error
    if err != nil { return 0, 0, err }
    distinct := len(rows)
    repeat := 0
    for _, r2 := range rows { if r2.Cnt > 1 { repeat++ } }
...
func (r *gormAnalyticsRepository) GetSlotUtilization(userID uuid.UUID, startDate, endDate time.Time) (int64, int64, error) {
    ...
    err := r.db.Table("bookings").
        Joins("JOIN appointments ON bookings.appointment_id = appointments.id").
        Where("appointments.owner_id = ? AND appointments.type IN ('single','group') AND bookings.date BETWEEN ? AND ?", userID, startDate, endDate).
        Count(&total).Error
    if err != nil { return 0, 0, err }
    err = r.db.Table("bookings").
        Joins("JOIN appointments ON bookings.appointment_id = appointments.id").
        Where("appointments.owner_id = ? AND appointments.type IN ('single','group') AND bookings.date BETWEEN ? AND ? AND bookings.available = ? AND bookings.status = ?", userID, startDate, endDate, false, "active").
        Count(&booked).Error
    if err != nil { return 0, 0, err }
...
func (r *gormAnalyticsRepository) GetAvgAttendeesPerBooking(userID uuid.UUID, startDate, endDate time.Time) (float64, error) {
    ...
    err := r.db.Raw(`
        ...
        FROM bookings
        JOIN appointments ON bookings.appointment_id = appointments.id
        WHERE appointments.owner_id = ? AND bookings.created_at BETWEEN ? AND ? AND bookings.status = 'active'
...
func (r *gormAnalyticsRepository) GetPartyCapacity(userID uuid.UUID, startDate, endDate time.Time) (int64, int64, error) {
    type row struct{ Total int64; Used int64 }
    var o row
    err := r.db.Raw(`
...
func (r *gormAnalyticsRepository) GetLeadTimeStatsHours(userID uuid.UUID, startDate, endDate time.Time) (float64, float64, error) {
    ...
    err := r.db.Raw(`
        ...
        WHERE appointments.owner_id = ? AND bookings.created_at BETWEEN ? AND ? AND bookings.status = 'active'
    `, userID, startDate, endDate).Scan(&o).Error
    return o.Avg, o.Median, err
}

type DateCount struct { Date string; Count int }
type KeyCount struct { Key string; Count int }
type TopAppointmentRow struct { AppCode string; Title string; Bookings int; CapacityUsagePercent float64 }
    ...
    err := r.db.Raw(`
        WITH base AS (
            SELECT appointments.id, appointments.app_code, appointments.title,
                   COUNT(CASE WHEN bookings.status = 'active' THEN 1 END) AS bookings,
                   CASE 
